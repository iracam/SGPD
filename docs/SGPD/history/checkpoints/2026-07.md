...
### 2026-07-30 — Fase 5: pendências e evidências

```text
Data: 2026-07-30
Responsável: Codex
Fase: Checkpoint 5 — Pendências e evidências
O que foi concluído: pendências e seus itens como entidades próprias; comentários append-only; ciclo de regularização versionado; classificação e bloqueio da conclusão; evidências privadas com validação de extensão, MIME e assinatura, limite configurável, nome aleatório, SHA-256, upload idempotente e download auditado; autorização por setor, DP no escopo e SuperAdmin; API e SPA mobile first.
Diagnóstico: tarefas já possuíam flags históricas de pendência/evidência, locks, versão e idempotência, mas o backend rejeitava FILE/requires_evidence e não existiam models, storage ou endpoints da Fase 5. O storage configurado ainda não estava registrado no STORAGES nem protegido por um contrato de upload/download.
Decisões: executar as ADRs 007, 008, 011, 014, 019, 025–028, 039, 043 e 044 sem nova ADR; manter valores e decisões fora do incremento; implementar somente ABERTA, EM_REGULARIZACAO, REGULARIZADA e ENCERRADA; aceitar inicialmente PDF, PNG e JPEG até 10 MiB; nunca expor caminho privado; remover o novo arquivo se a transação falhar; exigir comentário em toda transição; permitir responsável vigente, DP no escopo e SuperAdmin sem criar vínculos artificiais.
Riscos: retenção continua sem homologação; limite e catálogo de formatos precisam de homologação operacional; validação de assinatura é deliberadamente restrita aos três formatos iniciais; a interface ainda exige conferência visual nos cinco breakpoints; prontidão e liberação formal permanecem na Fase 8.
Pendências: homologar retenção, formatos/tamanho e experiência visual; não iniciar valores ou decisão financeira antes da Fase 6 autorizada.
Próximo passo: executar homologação funcional de registrar pendência, comentar, regularizar, anexar/baixar evidência e concluir a tarefa nos cinco breakpoints.
Comandos executados: leitura integral da documentação obrigatória e ADRs relacionadas; testes direcionados e completos; Ruff check/format; Mypy; Django check; makemigrations check; Vitest; build Angular; sqlmigrate Oracle; migrate; validação somente leitura de tabelas, constraints, índices e plano final.
Arquivos alterados: novas apps pending_items e evidence; models, services, API, storage, admin e migrations; integração de offboarding; configuração Django e .env.example; feature Angular de tarefas e configuração de API; testes backend/frontend; README.md e documentação canônica/checkpoint.
Testes: 336 testes backend e 75 testes frontend passaram; Ruff e format sem erros; Mypy sem erros em 162 arquivos; Django check somente com o warning conhecido AD900 de LDAP simples; models e migrations sem divergência; build de produção concluído. offboarding.0005, pending_items.0001 e evidence.0001 foram aplicadas no Oracle DEV; SGPD_EVIDENCE, SGPD_PENDING_COMMENT, SGPD_PENDING_ITEM e SGPD_PENDING_ITEM_LINE existem, 49 constraints estão ENABLED/VALIDATED, não há índice inválido e o plano final está vazio. Nenhum objeto ou dado do Senior HCM foi consultado ou alterado.
```

### 2026-07-30 — Replay da Fase 5 compatível com Oracle

```text
Data: 2026-07-30
Responsável: Codex
Fase: Checkpoint 5 — Correção Oracle
O que foi concluído: remoção do LIMIT/OFFSET implícito nas consultas bloqueadas de replay idempotente de pendências e evidências; ambas agora materializam o conjunto protegido pela constraint única sem fatiamento.
Diagnóstico: QuerySet.first() adicionava FETCH FIRST à consulta select_for_update(). O backend Oracle rejeita essa combinação com NotSupportedError antes do upload em /api/v1/evidence/.
Decisões: reutilizar o padrão Oracle já adotado pelas tarefas, listando o queryset bloqueado e escolhendo a primeira linha em memória; preservar locks, constraint única, idempotência e resposta da API.
Riscos: novos usos de first(), last() ou slice após select_for_update() podem reintroduzir a incompatibilidade; a regressão agora falha explicitamente nesse caso.
Pendências: nenhuma no escopo da correção.
Próximo passo: repetir o upload pela tela de tarefas.
Comandos executados: busca global por select_for_update com first/last; testes direcionados e completos; Ruff check/format; Mypy; diff check; SELECT FOR UPDATE equivalente no Oracle DEV sem DML.
Arquivos alterados: apps/evidence/services.py; apps/pending_items/services.py; tests/test_pending_items_evidence.py; ARCHITECTURE.md; CHECKPOINT.md; MANIFEST.json.
Testes: 337 testes backend passaram; 27 regressões direcionadas passaram; Ruff, format e Mypy sem erros. A consulta bloqueada real no Oracle DEV retornou zero linhas sem NotSupportedError. Nenhum DML foi executado e nenhum objeto do Senior HCM foi consultado ou alterado.
```

### 2026-07-30 — Card Em Aberto no hub de processos

```text
Data: 2026-07-30
Responsável: Codex
Fase: Checkpoint 4 — Painel inicial do DP
O que foi concluído: o hub /fe/processos passou a exibir o card expansível Em Aberto no mesmo modelo de Concluídos, com contagem, dados do processo, prazo e tarefas sob demanda.
Diagnóstico: processos iniciados com tarefas pendentes não apareciam em nenhum card do hub; somente rascunhos e processos operacionalmente concluídos possuíam projeção própria.
Decisões: classificar no backend como Em Aberto apenas o processo INICIADO com ao menos uma tarefa não concluída; manter Em Aberto e Concluídos mutuamente exclusivos; preservar os escopos DP, a autoridade global de SuperAdmin e a consulta Oracle por EXISTS, sem agregação sobre NCLOB.
Riscos: o hub limita cada grupo aos 50 processos mais novos e cada expansão a 100 tarefas; paginação visual permanece pendente.
Pendências: conferir visualmente a grade nos cinco breakpoints homologados.
Próximo passo: homologar a passagem automática do card Em Aberto para Concluídos ao concluir a última tarefa.
Comandos executados: testes backend e frontend direcionados e completos; Ruff check/format; Mypy; Django check; makemigrations check; build Angular; busca por max-width; diff check.
Arquivos alterados: API, serializer, services e testes de offboarding; feature Angular de processos; README.md; ARCHITECTURE.md; REQUIREMENTS.md; ROADMAP.md; CHECKPOINT.md; MANIFEST.json.
Testes: 337 testes backend e 75 testes frontend passaram; 18 regressões direcionadas de workflow e a regressão Angular do hub passaram; Ruff, format e Mypy sem erros; Django check somente com o warning conhecido AD900 de LDAP simples; models e migrations sem divergência sob o settings de teste; build de produção concluído com bundle inicial de 497,15 kB; nenhuma consulta max-width foi introduzida; diff sem erros de whitespace. O check conectado tentou consultar o histórico de migrations, mas o Oracle DEV estava indisponível com ORA-12560. Não houve migration, alteração de schema, escrita no Oracle DEV nem acesso ao Senior HCM.
```

### 2026-07-30 — Regras de aplicabilidade e conclusão da Fase 3

```text
Data: 2026-07-30
Responsável: Claude
Fase: Fase 3 — Configuração funcional (RF-012)
O que foi concluído: entidade, service, API e SPA das regras de aplicabilidade, encerrando a última entrega pendente da Fase 3; o rascunho passou a devolver e pré-marcar os grupos sugeridos pelo snapshot.
Diagnóstico: a aplicabilidade automática existia apenas no DATA_MODEL; a ADR-039 restringira o recorte ao escopo organizacional do setor e o DP reconhecia manualmente, a cada processo, quais grupos publicados se aplicavam.
Decisões: ADR-046 — a regra sugere e não aplica, preservando a seleção explícita da ADR-039; união entre regras vigentes, com PRIORIDADE apenas ordenando a exibição; campo vazio é curinga; a regra aponta para o cabeçalho do grupo e resolve sua versão publicada vigente, por isso não é versionada; a sugestão é limitada aos grupos disponíveis pelo escopo do setor. Descartada a semântica de maior prioridade vence, que esconderia grupos do DP sem sinal visível.
Riscos: o catálogo de regras segue vazio e a semântica de união só será validada na homologação funcional.
Pendências: cadastrar e homologar o catálogo de regras; conferir visualmente o editor e o bloco de sugestão nos cinco breakpoints.
Próximo passo: cadastrar as primeiras regras com o responsável funcional.
Comandos executados: sqlmigrate para revisão do SQL Oracle; testes backend e frontend completos; Ruff check/format; Mypy; Django check; makemigrations check; build Angular; showmigrations e consulta somente leitura a user_constraints/user_indexes após a aplicação.
Arquivos alterados: models, services, serializers, api, urls, admin e migration 0006 de templates_engine; api de offboarding; tests/test_applicability_rules.py; features Angular workflow-config e processo-rascunho; api.config.ts; README.md; ARCHITECTURE.md; CONTEXT.md; DATA_MODEL.md; DECISIONS.md; DECISIONS_INDEX.md; REQUIREMENTS.md; ROADMAP.md; CHECKPOINT.md; MANIFEST.json.
Testes: 355 testes backend e 80 testes frontend passaram, sendo 18 regressões novas de aplicabilidade no backend e 4 no frontend; Ruff, format e Mypy sem erros; Django check sem issues; models e migrations sem divergência sob o settings de teste; build de produção concluído. A migration templates_engine.0006 foi aplicada no Oracle DEV pelo responsável; a verificação somente leitura confirmou 20 constraints e 3 índices ENABLED/VALIDATED, com a tabela vazia. Nenhum DML foi executado e o Senior HCM não foi consultado.
```

### 2026-07-30 — Remoção da busca de templates na SPA

```text
Data: 2026-07-30
Responsável: Claude
Fase: Fase 3 — Configuração funcional
O que foi concluído: remoção do card de busca por nome do catálogo de templates em /fe/workflow-config, junto do estado e do método que só existiam para ele.
Diagnóstico: o volume de templates cadastrados não justifica um filtro; a busca duplicava a lista já carregada e mantinha dois sinais em paralelo (templates e templatesVisiveis) que precisavam ser atualizados juntos a cada operação.
Decisões: exibir o catálogo completo a partir de um único sinal; preservar o parâmetro q na API, que continua contratado e testado no backend.
Riscos: com muitos templates, a lista completa pode ficar longa; a busca volta pela API sem mudança de contrato caso o volume cresça.
Pendências: nenhuma no escopo da remoção.
Próximo passo: cadastrar as primeiras regras de aplicabilidade com o responsável funcional.
Comandos executados: testes frontend completos; build Angular.
Arquivos alterados: feature Angular workflow-config (html, ts, scss, service e spec); ARCHITECTURE.md; REQUIREMENTS.md; MANIFEST.json.
Testes: 80 testes frontend passaram; build de produção concluído com bundle inicial de 497,23 kB. Nenhuma alteração de backend, schema ou dado.
```

"""Input contracts for the validation-sector API."""
...
class SectorResponsibleScopeSerializer(serializers.Serializer[dict[str, Any]]):
    scope_type = serializers.ChoiceField(choices=ScopeType.choices)
    company_code = serializers.IntegerField(min_value=1, required=False, allow_null=True)
    branch_code = serializers.IntegerField(min_value=1, required=False, allow_null=True)
...
class SectorResponsibleAssignSerializer(SectorResponsibleScopeSerializer):
...
class SectorResponsibleUpdateSerializer(serializers.Serializer[dict[str, Any]]):
    version = serializers.IntegerField(min_value=1)
    ...
    def validate(self, attrs: dict[str, Any]) -> dict[str, Any]:
        valid_until = attrs.get("valid_until")
        if valid_until is not None and valid_until <= attrs["valid_from"]:
            raise serializers.ValidationError(
                {"valid_until": "A validade final deve ser posterior à inicial."}
            )
        return attrs
...
class SectorResponsibleRevokeSerializer(serializers.Serializer[dict[str, Any]]):
    version = serializers.IntegerField(min_value=1)
    reason = serializers.CharField(max_length=2000, trim_whitespace=True)
